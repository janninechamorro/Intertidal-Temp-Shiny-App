...
=======
  read_csv(here:here("logger_data","robomussel_data", file_name_csv), 
           col_names=TRUE, 
           col_types=cols(.default=col_character())) %>% 
    mutate(Time_GMT=as.POSIXct(Time_GMT, "%m/%d/%Y %H:%M:%S", tz="GMT"), 
           local_datetime=with_tz(Time_GMT, tz="_Angeles"),
           year=substr(local_datetime, 7,10), 
           month=substr(local_datetime,1,2), 
           day=substr(local_datetime,4,5),
>>>>>>> c91b2f85405a9e854ec8984174f21fdfc684715a
           year=as.factor(year),
